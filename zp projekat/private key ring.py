...
def send_message():
    ...
    if encrypt_var.get() == 1:
        ...
        encrypted_aes_key = message_receiver_public_key.encrypt(
            aes_key,
            asym_padding.OAEP(  # Use OAEP padding (recommended for RSA encryption)
        ...
        # Concatenate them with a delimiter and add message_receiver_number
        # Using a pipe '|' as a delimiter, you can use any delimiter that you prefer
    ...
    # samo se trazio potpis poruke + poruka
    # combined_message = signedMessage + delimiter1 + number_bytes + delimiter2 + message.encode('utf-8')

    write_message_to_file_bite(combined_message)


def write_message_to_file_bite(concatenated_message):
    ...
    try:
        ...
        messagebox.showinfo("Info", f"Message successfully written to {file_path}")
    except Exception as e:
        messagebox.showwarning("Info", f"ERROR. Message not sent: {str(e)}")
...
def receive_message():
    ...
    if sign_var.get() == 1 and encrypt_var.get() == 0:
        receive_signed_message()
        return

    elif (encrypt_var.get() == 1 and sign_var.get() == 1) or (encrypt_var.get() == 1 and sign_var.get() == 0):
        receive_signed_and_encrypted_message()
        return
...
def receive_signed_message():
    ...
    try:
        # Open the file in binary mode to read the data
        with open(file_path, 'rb') as file:
            ...
            # Split the binary message by the first delimiter
            parts = concatenated_message.split(delimiter1)

            if len(parts) != 2:
                raise ValueError("Invalid message format. Delimiter not found or incorrect.")

            signedMessage = parts[0]  # This is the signed message (binary)
            remainder = parts[1]

            # Split the remainder by the second delimiter
            parts = remainder.split(delimiter2)

            if len(parts) != 2:
                raise ValueError("Invalid message format. Delimiter not found or incorrect.")

            number = int(parts[0].decode('utf-8'))  # Decode the number from binary to string, then convert to int
            originalMessage = parts[1].decode('utf-8')  # Decode the original message from binary to string
            ...
            # Load the sender's public key (based on the number)
            print("Number:", number)
            sender_public_key = load_rsa_public_key(public_key_ring[number].public_key)
            sender_email = public_key_ring[number].email
            ...
            try:
                ...
                messagebox.showinfo("Message Signature", f"Message fits the signature\n"
                                                         f"Message: {originalMessage}\n"
                                                         f"Sender: {sender_email}")
            except Exception as e:
                messagebox.showwarning("Message Signature", "Message does not match the signature")

    except Exception as e:
        print(f"Failed to read or process the message: {e}")

def receive_signed_and_encrypted_message():
    ...
    #       sender_public_key = load_rsa_public_key(public_key_ring[number].public_key) #ako se svi kljucevi izvoze
            sender_public_key = load_rsa_public_key(public_key_ring[0].public_key)

    #       sender_email = public_key_ring[0].email #ako se svi kljucevi izvoze
            ...
            messagebox.showwarning("Message", f"message: {decrypted_combined_message.decode('utf-8')}")
